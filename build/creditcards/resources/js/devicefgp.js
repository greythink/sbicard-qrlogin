...
function fingerprint_connectiontype() {
    "use strict";
    ...
    try {
		...
		// return NA if navigator.connection object does not apply to this device
        return strOnError;
    }
    return strOut;
}
...
function fingerprint_cookieenabled() {
    "use strict";
...
try {
    ...
    if ("function" !== typeof nt_vc_context) nt_vc_output = "Not available";
        ...
        var r_key_state = "ac-state";
        delete nt_vc_output[r_key_state];// this is removed as Chrome sometimes stop the audio context
        nt_vc_output = window.JSON.stringify(nt_vc_output, undefined, 2);
    }
} catch (g) {
    nt_vc_output = 0;
}
return sha256(nt_vc_output);
}

function fingerprint_networktype(){
var n_type;
...
if (connection === null) {
   ...
   n_type = connection.effectiveType;
}
...
/*props.browser = exec_func("browser");
props.flash = exec_func("flash");
props.canvas = exec_func("canvas");//fingerprint_canvas();
props.connection = fingerprint_connection();
props.cookie = fingerprint_cookie();
props.display = fingerprint_display();
props.fontsmoothing = fingerprint_fontsmoothing();
props.fonts = fingerprint_fonts();
props.formfields = fingerprint_formfields();
props.java = fingerprint_java();
props.language = fingerprint_language();
props.silverlight = fingerprint_silverlight();
props.os = fingerprint_os();
props.timezone = fingerprint_timezone();
props.touch = fingerprint_touch();
props.plugins = fingerprint_plugins();
props.useragent = fingerprint_useragent();
props.truebrowser = fingerprint_truebrowser();
props.audio = fingerprint_audio();
props.network = fingerprint_network();
*/


function exec_func(function_name) {
	return eval("fingerprint_" + function_name + "()");
}
...
function getdevicefgp(fp){

var filter_props = fp.split(',');
var props = {};
var errorprops = '';

for( i=0 ; i < filter_props.length; i++){

	try{
		props[camelize(filter_props[i]).replace('-','')] = exec_func(filter_props[i].replace('-',''));
	}
	catch(err){
		if(errorprops == ''){
			errorprops = filter_props[i] ;
		} else{
			errorprops = errorprops + ',' + filter_props[i] ;
		  }
		}
}

if(errorprops != ''){
	props['errorProperty'] = errorprops;
	//console.log('error occured while evaluating - ' + errorprops  + ' properties');
}
